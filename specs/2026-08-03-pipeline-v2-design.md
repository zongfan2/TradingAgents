...
| D2 | Two market sessions: `cn` (A-shares+HK, 08:30 Asia/Shanghai) and `us` (08:30 America/New_York); IANA tz names only. Fire times are always converted from the session tz to host-local (the host is **America/Chicago** — 08:30 Shanghai is the previous local evening), and slot dates/weekdays resolve in the session tz | "CST"/"GMT" abbreviations are ambiguous; both markets wanted; assuming host tz would run cn 13–14h late (Codex review pt 2) |
